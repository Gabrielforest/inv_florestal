...
# comercial sem casca (vcomsc), processe o inventário florestal e responda os itens abaixo. Considere α = 5%. (100%)
...
arv <- read.csv2("listas/lista3_fustes.csv")
cub <- read.csv2("listas/cubagem_lista3.csv")
...
# dados de altura dominante -----------------------------------------------

library(dplyr)

# tabela de dados das dominantes:
dom <- subset(arv, arv$categoria == "Dominante")

# média das dominantes por parcela:
hd <- dom %>%
        group_by(parcela) %>% 
...
hd <- aggregate(
        list(hd = dom$ht),
        list(parcela = dom$parcela),
        mean, na.rm = TRUE)

# OU

fhdom <- function(x){
        x <- x[order(x$dap, decreasing = TRUE), ]
...
arv$hdom <- cmrinvflor::procv(arv, Reduce("rbind", lapply(split(arv, arv$parcela, fhdom), "hdom", by.x = "parcela"))) 
...
as.data.frame(cmrinvflor::estats_acs(vy = z, nt = N, sig = 5))
